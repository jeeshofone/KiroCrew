...
needed. Disabling or uninstalling the app counts as declaring nothing, which
collapses its open sockets to the always-delivered tier — including the log stream,
which is re-checked per send rather than only at subscribe. Widening does not work
