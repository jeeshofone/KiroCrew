...
_declared_cache: dict[str, tuple[float, frozenset[str]]] = {}
_declared_refreshing: set[str] = set()


def _read_declared_events(app: str) -> frozenset[str]:
    """Blocking read of *app*'s current ``permissions.events`` set.
    ...
    the intersection would keep honouring them. A disabled or uninstalled app
    declares nothing, which collapses an open socket to Tier 0.
    """
    try:
        if not is_app_enabled(app):
            return frozenset()
        manifest = get_app_manifest(app)
        if manifest is None:
            return frozenset()
        return build_allowed_event_set(list(manifest.permissions.events))
    except Exception:
...
def _schedule_declared_refresh(app: str) -> None:
    ...
    try:
        ...
        _declared_cache[app] = (time.monotonic(), _read_declared_events(app))
        return
    ...
    def _store(fut: Any) -> None:
        _declared_refreshing.discard(app)
        try:
            _declared_cache[app] = (time.monotonic(), fut.result())
        except BaseException:
...
def effective_allowed_events(app: str, connect_snapshot: frozenset[str]) -> frozenset[str]:
    ...
    if cached is not None:
        ...
        return connect_snapshot & cached[1]
    _schedule_declared_refresh(app)
