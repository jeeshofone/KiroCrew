...
async def _reconnect_muted(
    ...
    try:
        async with lock:
            ...
            async def _restore_mute() -> None:
                """Put the binding back the way the user left it: muted."""
                try:
                    await asyncio.to_thread(
...
async def _claim_and_seed(
    ...
    async def _release_claim() -> None:
        """Undo the claim AND the eviction it performed.

        Safe to restore the evicted bindings because the conversation lock is still
        held: nothing else can have claimed this location in the meantime, so we
        are putting back the state we ourselves displaced, not clobbering a rival.
        """
        try:
            if previous is not None:
                await asyncio.to_thread(
                ...
                if previous_paused:
                    # The rebind cleared the mute; put it back explicitly.
                    await asyncio.to_thread(
                        state.sessions.set_mirror_paused,
                        session_key,
                        True,
                ...
                await asyncio.to_thread(
                    state.sessions.clear_mirror_link, session_key, link.channel_type
                )
        except Exception:
            logger.debug("mirror-link could not release its claim", exc_info=True)
        for occupant, held, inbound, paused in evicted:
            try:
                await asyncio.to_thread(
                    state.sessions.set_mirror_link, occupant, held, accepts_inbound=inbound
                )
                if paused:
                    # Restoring the link alone would leave the occupant CONNECTED
                    # after a takeover that failed — the failed attempt would have
                    # silently un-muted a channel the user had muted.
                    await asyncio.to_thread(
                        state.sessions.set_mirror_paused,
                        occupant,
                        True,
                ...
                logger.debug(
                    "mirror-link could not restore evicted binding for %s",
                    occupant,
                    exc_info=True,
                )
    ...
    try:
        if confirmed:
            ...
            evicted = await asyncio.to_thread(
                state.sessions.replace_mirror_owner, session_key, link, accepts_inbound=True
            )
        else:
    ...
    # and the claim is released so a denied binding never persists.
