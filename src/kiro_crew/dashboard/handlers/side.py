    ...
    *,
    ...
    def _on_steer_consumed(snapshot: str) -> None:
        ...
        if side_at_start is None or slot._side is not side_at_start:
            return
        side_at_start.steer_settle(snapshot)
    ...
    if slot._side.last_run_id and not slot._side.is_complete:
        ...
        if body.get("steer"):
            ...
            if steer_id is not None:
                ...
                # has since ended. Reporting a demotion here would hide a
                # delivered question entirely.
                delivered = ledger == STEER_CONSUMED or (
                    ledger == STEER_PENDING
                    and side_before.last_run_id == run_before
                    and not side_before.is_complete
                )
                if delivered:
                    side_before.append_user(question, steer=True)
                    broadcast_side_result(
                        state,
                        slot_key=slot.key,
                        run_id=run_before,
