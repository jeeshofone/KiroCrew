...
async def api_terminal_ws(request: web.Request) -> web.WebSocketResponse | web.Response:
    ...
    # Resolve the redaction mode BEFORE reserving the placeholder. It is latched
    # per session, and this is the one config read, so it must not sit between
    # the placeholder assignment and the real session object: that window is
    # already observable (concurrent requests and the title poller both skip a
    # `None` slot), and adding a suspension point inside it made a disconnect
    # racing session setup hit `None.winpty`.
    redact_output = await _redacts_output(request) if not existing else True
