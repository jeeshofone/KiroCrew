...
async def api_chat_slot_migrate_remote(request: web.Request) -> web.Response:
    ...
    reserved_key = _hold_migration(state, slot, name)
    try:
        ...
        def _refuse_if_turn_started(retired_loop: "NudgeLoop | None" = None) -> None:
            ...
            # mutation at all (see `_bundled_sid`). The DESTINATION's identity is
            # rechecked the same way: a destination closed (and its key released
            # or reused) inside the persist+archive window must not let this
            # migration report success against a key that does not name it.
            if (
                state._slots.get(name) is not slot
                or state._slots.get(new_slot.key) is not new_slot
                or _anchors_moved()
        ...
        _release_migration(state, slot, name, reserved_key)
...
def _destination_received_work(dest: _ChatSlot) -> bool:
    ...
    guard (``_migrate_refusal``) applies. And so does an ARMED automation: a
    user who opened the published tab and started a monitor loop on it has
    given the slot a job, and popping the slot would leave the loop behind to
    rehydrate the closed transcript and run it unattended beside the source.
    Every rollback arm asks this immediately before it would pop the
    destination, and keeps it live when the answer is yes."""
    return (
        bool(dest.messages)
        or dest.running
        ...
        or _nudge_loop_armed_on(dest.key)
    )


def _nudge_loop_armed_on(slot_key: str) -> bool:
    """Is an auto-nudge loop registered for *slot_key* right now?

    Synchronous registry lookup, the same one the close path resolves before
    retiring a loop; it consults the live registry, not the slot. No service
    means no loops can exist. A FAILED lookup reads as "armed": the caller is
    deciding whether a pop is safe, and an unknown answer must fail toward
    keeping the slot.
    """
    try:
        from kiro_crew.autonudge import (
        ...
        logger.warning("autonudge lookup for migration rollback failed", exc_info=True)
        return True
    if service is None:
        return False
    try:
        return service.get_by_slot(slot_key) is not None
    except Exception:
        logger.warning("autonudge loop lookup for %s failed", slot_key, exc_info=True)
        return True


def _hold_migration(state: DashboardState, slot: _ChatSlot, name: str) -> str:
    """Admit a migration of *slot*: flag the slot and RESERVE its key.

    Returns the session key reserved on the shared ``SessionManager`` so the
    caller can hand exactly that key back to :func:`_release_migration` —
    the release must never recompute it, or a slot whose routing changes
    mid-flight would leave the reservation behind and block every channel
    resume of the key until restart.
    ...
    The same reservation is mirrored onto the shared ``SessionManager``
    (:meth:`SessionManager.mark_migrating`) under the SESSION key
    (``effective_session_key(slot)``, ``dashboard:<name>`` for a plain slot):
    channel resume binders live outside the dashboard and attach a channel to
    that key BY NAME, so they need to see the admission too — a bind landing
    between the pop and the stamp would otherwise fork the conversation the
    crew is carrying forward. The dashboard's own mint gate keeps the bare slot
    key in ``state._migrating_keys``; the two namespaces are deliberately kept
    apart, one per consumer.
    """
    slot._migrating = True
    state._migrating_keys[name] = str(getattr(slot, "_app", "") or "")
    reserved_key = effective_session_key(slot)
    _mark = getattr(state.sessions, "mark_migrating", None)
    if callable(_mark):
        _mark(reserved_key)
    return reserved_key


def _release_migration(
    state: DashboardState, slot: _ChatSlot, name: str, reserved_key: str
) -> None:
    """Undo :func:`_hold_migration` — on success (stamp durable) or any failure.

    *reserved_key* is the value :func:`_hold_migration` returned: the release
    unmarks exactly what the admission marked.
    """
    slot._migrating = False
    state._migrating_keys.pop(name, None)
    _unmark = getattr(state.sessions, "unmark_migrating", None)
    if callable(_unmark):
        _unmark(reserved_key)
...
def migrated_key_mint_decision(
    ...
    if not isinstance(migrated, dict) or not is_migrated_transcript(migrated):
        return None
...
def _migrate_refusal(
    ...
    # A question card still showing: a BLOCKING card implies a parked turn
    # (caught by the in-flight arm below), but a stateless card posted through
    # ``interaction_coordinator.post_card`` outlives its turn with
    # ``running=False`` and is retired only by the next user message on THIS
    # key. The answer it is waiting for would land on a source the archive
    # locks read-only, and the card itself is not in the bundle. Refuse until
    # the card is answered or dismissed.
    if slot._question_pending:
        return (
            "a question card is waiting for an answer",
            "migrate_question_pending",
        )
...
async def close_slot(
    ...
    *,
    ...
    try:
        await _close_slot(state, slot, name, pre_pop_check=pre_pop_check, migrated=migrated)
    finally:
