...
test("opclass: view ops are the read/show set", () => {
  for (const op of ["navigate", "back", "snapshot", "screenshot", "console", "wait_for"]) {
    assert.strictEqual(agentOpClass(op), "view", `${op} should be view-class`);
  }
});

test("opclass: input injection and script execution are operate ops", () => {
  for (const op of ["click", "type", "press_key", "hover", "select_option", "evaluate"]) {
