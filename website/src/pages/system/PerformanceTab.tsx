...
function StorageEntryRow({ onOpen }: { onOpen: () => void }) {
  ...
  const gb = (n: number) => `${fmtNumber(n / 1e9, { maximumFractionDigits: 1 })} GB`
        ...
        <div className="text-[11px] text-muted mt-0.5">
          {data
            ? i18nT('pages.performanceTab.session_storage_detail', {
                ...
                reclaimable: gb(data.reclaimable_bytes),
              })
            : i18nT('pages.performanceTab.session_storage_measuring')}
        </div>
      </div>
      {data && (
        <span className="text-[12px] text-text font-mono tabular-nums">{gb(data.total_bytes)}</span>
      )}
