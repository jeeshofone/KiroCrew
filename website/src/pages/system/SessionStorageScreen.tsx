...
function fmtBytes(bytes: number): string {
  if (bytes >= 1e9) return `${fmtNumber(bytes / 1e9, { maximumFractionDigits: 2 })} GB`
  if (bytes >= 1e6) return `${fmtNumber(bytes / 1e6, { maximumFractionDigits: 1 })} MB`
  if (bytes >= 1e3) return `${fmtNumber(bytes / 1e3, { maximumFractionDigits: 0 })} kB`
