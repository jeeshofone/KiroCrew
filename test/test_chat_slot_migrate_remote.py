...
class TestSuccess:
    ...
    async def test_fresh_session_mints_an_empty_peer_slot_instead_of_a_bundle(
        ...
        # The empty source had no metadata line and no messages, so the closed
        # save has no window to write; the migrated stamp must still land as a
        # durable tombstone, or a restart re-mints a writable local fork.
        assert "s1" not in state._slots
        meta, readable = state.conversation_log.get_metadata_status("dashboard:s1")
        assert readable and meta.get("closed") is True
        assert meta.get("migrated") == {"instance_id": "crew-a", "remote_key": "peer-fresh-1"}
        async with TestClient(TestServer(_make_app(state))) as client:
...
class TestFailureOrdering:
    ...
    @pytest.mark.asyncio
    async def test_a_destination_with_an_armed_nudge_loop_survives_a_failed_persist(
        self, tmp_path, monkeypatch
    ):
        """A user who opened the published tab and armed a monitor loop on it
        gave the slot a job. Popping it would orphan the loop, which rehydrates
        the closed transcript on its next fire and runs it unattended beside the
        source; the rollback keeps the destination live instead."""
        ...
        class _Registry:
            def get_by_slot(self, slot_key):
                return armed.get(slot_key)

        monkeypatch.setattr(autonudge, "get_instance", lambda: _Registry())

        def _loop_armed_then_write_fails(history_key, fields):
            if fields.get("executor") == "remote":
                dest_key = next(k for k in state._slots if k != "s1")
                armed[dest_key] = object()  # a loop registered for the destination
                raise OSError("disk full")
            return real_update(history_key, fields)

        monkeypatch.setattr(state.conversation_log, "update_metadata", _loop_armed_then_write_fails)
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.post(
                "/api/chat/slots/s1/migrate-remote", json={"instance_id": "crew-a"}
            )
            ...
            assert (resp.status, data["code"]) == (409, "migrate_split")
        assert "s1" in state._slots
        assert data["key"] in state._slots and state._slots[data["key"]]._dirty
...
class TestArchiveWindow:
    ...
    def test_the_release_unmarks_exactly_the_key_the_admission_marked(self, tmp_path):
        """The shared reservation is keyed by the session key captured at
        admission. If the slot's routing changes mid-flight the release still
        clears that captured key; recomputing it would leave the reservation
        behind and block every channel resume of the key until restart."""
        state = _make_state(tmp_path)
        slot = state.get_or_create_slot("s1")
        reserved = chat_handlers._hold_migration(state, slot, "s1")
        assert reserved == "dashboard:s1"
        state.sessions.mark_migrating.assert_called_once_with("dashboard:s1")
        slot.linked_session_key = "slack:123.456"  # routing changed in flight
        chat_handlers._release_migration(state, slot, "s1", reserved)
        state.sessions.unmark_migrating.assert_called_once_with("dashboard:s1")
    ...
    async def test_send_and_resume_inside_the_pop_to_commit_window_are_refused(
        ...
        async def _save_probing_the_window(state_, slot_, *a, **kw):
            if kw.get("closed") and slot_ is slot:
                ...
                # The channel binders' view of the same window: the shared
                # session manager carries the reservation until the stamp lands.
                state_.sessions.mark_migrating.assert_called_once_with("dashboard:s1")
                state_.sessions.unmark_migrating.assert_not_called()
...
class TestConsistencyAnchor:
    ...
    @pytest.mark.asyncio
    async def test_a_destination_closed_inside_the_window_aborts_the_archive(
        self, tmp_path, monkeypatch
    ):
        """The destination's identity is re-checked at the same boundary as the
        source's: a destination the owner closes inside the persist+archive
        window (its key released, or reused by another slot) must not let the
        migration report success against a key that does not name it. The
        source stays live and unarchived."""
        state = _make_state(tmp_path)
        slot = state.get_or_create_slot("s1")
        _seed_conversation(slot)
        _stub_transfer(state, monkeypatch)
        ...
        def _destination_closed_during_persist(history_key, fields):
            if fields.get("executor") == "remote":
                dest_key = next(k for k in state._slots if k != "s1")
                state._slots.pop(dest_key)  # the owner closed it; the key is free
            return real_update(history_key, fields)

        monkeypatch.setattr(
            state.conversation_log, "update_metadata", _destination_closed_during_persist
        )
        async with TestClient(TestServer(_make_app(state))) as client:
            resp = await client.post(
                "/api/chat/slots/s1/migrate-remote", json={"instance_id": "crew-a"}
            ...
            data = await resp.json()
            assert (resp.status, data["code"]) == (409, "turn_in_flight")
        assert "s1" in state._slots and state._slots["s1"].migrated is None
        assert state._slots["s1"].executor != "remote"
...
class TestRoundSeventeen:
    ...
    @pytest.mark.asyncio
    async def test_stateless_question_card_refuses_at_entry(self, tmp_path):
        """A non-blocking card posted through ``interaction_coordinator.post_card``
        outlives its turn (``running`` is False) and is retired only by the next
        user message on this key; its answer would land on a source the archive
        locks read-only, and the card is not part of the bundle."""
        state = _make_state(tmp_path)
        slot = state.get_or_create_slot("s1")
        slot._question_pending["card-1"] = {"ts": 1.0, "blocking": False}
        ...
        async with TestClient(TestServer(_make_app(state))) as client:
            ...
            assert resp.status == 409
            assert (await resp.json())["code"] == "migrate_question_pending"
        assert "s1" in state._slots
        assert "card-1" in state._slots["s1"]._question_pending
